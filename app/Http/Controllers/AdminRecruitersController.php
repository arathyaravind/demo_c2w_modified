<?php namespace App\Http\Controllers;
	...
	class AdminRecruitersController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Password','name'=>'password','type'=>'password','validation'=>'min:3|max:32','width'=>'col-sm-10','help'=>'Minimum 5 characters. Please leave empty if not changing.'];
	    ...
	    */
	    ...
	    public function getDetail($id)	{
			...
			$recruiter = DB::table($this->table)->where($this->primary_key,$id)->first();

			// Get joborders
			$joborders = DB::select(
				"select jo.*,
				(select c.name from companies c where c.id = jo.company_id) as 'company_name'
				from job_orders jo where recruiter = " . $id
			);
			$recruiter->jo_count = count($joborders); 
			
			// TODO Tasklist of Recruiter
			...
			$this->cbView('override.recruiters.detail',compact('row','page_menu','page_title','command','id','recruiter','joborders'));
		}
