@extends('crudbooster::admin_template')
 <title>{{ isset($page_title) ? cbGetsetting('appname').': '.strip_tags($page_title) : "C2W: Offer Made List" }}</title>
@section('content')
@include('override.details_dashboard._styles')
<div class="container-fluid">
    <div class="row">
        <div class="table-responsive">
        <!-- 	<form method="get" action='/admin/offercandidateListPdfView/' target="_blank" class="form-horizontal">
			...
			<form method="get" action='/admin/offercandidateListCsvView' target="_blank" class="form-horizontal">
				<button type ="submit" class="btn btn-md btn-primary btn-xs pull-right button_margin">
					Export CSV
				</button>
				<input type="hidden" name="fromDate" value="{{ $_REQUEST['fromDate'] }}">
				...
				<tbody>
					...
					@foreach($jobOrderApplicants as $jobOrderApplicant)
						<tr>
							...
							<td> @if(!empty($jobOrderApplicant->offer_confirmation_date))
									{{date('d/m/Y',strtotime($jobOrderApplicant->offer_confirmation_date))}}
								@else
								  @if(!empty($jobOrderApplicant->new_offer_confirmation_date))
									{{date('d/m/Y',strtotime($jobOrderApplicant->new_offer_confirmation_date))}}
								  @endif
								@endif		
							</td>
							...
							<td>@if(!empty($jobOrderApplicant->offer_confirmation_date))
								...
								@if($jobOrderApplicant->offer_confirmation_date)
									<span class="jo-date-highlight">Confirm on <b>{{ date("d/m/Y", strtotime($jobOrderApplicant->offer_confirmation_date)) }}</b></span>
								@endif 
								@else
								{{'Confirm Offer'}}
								@if($jobOrderApplicant->new_offer_confirmation_date)
									<span class="jo-date-highlight">Confirm on <b>{{ date("d/m/Y", strtotime($jobOrderApplicant->new_offer_confirmation_date)) }}</b></span>
								@endif
								@endif
							</td>
						</tr>
...
var html = '<h1><i class="fa fa-dashboard"></i> Offer Made List</h1>';
