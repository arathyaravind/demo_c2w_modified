@extends('crudbooster::admin_template')
 <title>{{ isset($page_title) ? cbGetsetting('appname').': '.strip_tags($page_title) : "C2W: Interview Scheduled List" }}</title>
@section('content')
@include('override.details_dashboard._styles')
<div class="container-fluid">
    <div class="row">
        <div class="table-responsive">
        	<!-- <form method="get" action='/admin/interviewcandidateListPdfView' target="_blank" class="form-horizontal" >
        				...
        				<form method="get" action='/admin/interviewcandidateListCsvView' target="_blank" class="form-horizontal" >
        					<button type ="submit" class="btn btn-md btn-primary btn-xs pull-right button_margin">
        						Export CSV
        					</button>
        					<input type="hidden" name="fromDate" value="{{ $_REQUEST['fromDate'] }}">
			...
			<table class="table table-striped ">
				...
				<tbody>
					...
					<?php  ?>
							...
							<td>@if(!empty($jobOrderApplicant->interview_date))
									{{date('d/m/Y',strtotime($jobOrderApplicant->interview_date))}}
								@else
								   @if(!empty($jobOrderApplicant->new_interview_date))
								    {{date('d/m/Y',strtotime($jobOrderApplicant->new_interview_date))}}
								    @endif
								 @endif
							</td>
							...
							<td> 
								@if(!empty($jobOrderApplicant->interview_date)&&($jobOrderApplicant->next_action=='Confirm Interview Schedule'))
								{{$jobOrderApplicant->next_action}}
								@if($jobOrderApplicant->interview_date)
									<span class="jo-date-highlight jo-date-highlight-gray">Interview round: <b>{{ $jobOrderApplicant->interview_round }}</b></span>
									<span class="jo-date-highlight jo-date-highlight-blue">Interview on <b>{{ date("d/m/Y h:i:s A", strtotime($jobOrderApplicant->interview_date)) }}</b></span>
									@endif
								@else
								{{'Confirm Interview Schedule'}}
								@if($jobOrderApplicant->new_interview_date)
									<span class="jo-date-highlight jo-date-highlight-gray">Interview round: <b>{{ $jobOrderApplicant->interview_round }}</b></span>
									<span class="jo-date-highlight jo-date-highlight-blue">Interview on <b>{{ date("d/m/Y h:i:s A", strtotime($jobOrderApplicant->new_interview_date)) }}</b></span>
									@endif
								@endif
								</td>
						</tr>
...
var html = '<h1><i class="fa fa-dashboard"></i> Interview Scheduled List</h1>';
