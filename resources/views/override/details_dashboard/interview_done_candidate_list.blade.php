@extends('crudbooster::admin_template')
 <title>{{ isset($page_title) ? cbGetsetting('appname').': '.strip_tags($page_title) : "C2W: Interview Done List" }}</title>
@section('content')
@include('override.details_dashboard._styles')
<div class="container-fluid">
    <div class="row">
        <div class="table-responsive">
        	<!-- <form method="get" action='/admin/interview-done-candidates-pdf-view' target="_blank" class="form-horizontal">
			...
			<form method="get" action='/admin/interview-done-candidates-csv-view' target="_blank" class="form-horizontal">
				<button type ="submit" class="btn btn-md btn-primary btn-xs pull-right button_margin">
					Export CSV
				</button>
				<input type="hidden" name="fromDate" value="{{ $_REQUEST['fromDate'] }}">
				...
				<tbody>
					...
					@foreach($jobOrderApplicants as $jobOrderApplicant)
						<tr>
						    ...
						    @endif</td>
							<td>@if($jobOrderApplicant->next_action=='Get Interview Feedback'&& $jobOrderApplicant->secondary_status=='Interview Done')
								{{$jobOrderApplicant->next_action}}
								@if($jobOrderApplicant->interview_followup_date)
									<span class="jo-date-highlight">Follow up on <b>{{ date("d/m/Y", strtotime($jobOrderApplicant->interview_followup_date)) }}</b></span>
								@endif
								@else
								{{'Get Interview Feedback'}}
								@if($jobOrderApplicant->new_interview_followup_date)
									<span class="jo-date-highlight">Follow up on <b>{{ date("d/m/Y", strtotime($jobOrderApplicant->new_interview_followup_date)) }}</b></span>
								@endif
								@endif 
							</td>
						</tr>
...
var html = '<h1><i class="fa fa-dashboard"></i> Interview Done List</h1>';
