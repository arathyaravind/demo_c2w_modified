...
@section('content')
<link href="https://cdnjs.cloudflare.com/ajax/libs/select2-bootstrap-css/1.4.6/select2-bootstrap.min.cssno" rel="stylesheet" />
	...
	<div class="row">
		<div class="panel panel-default jo-panel">
			<div class="panel-body">
				<form class="form-horizontal" method="post" id="form" enctype="multipart/form-data" novalidate="" action='{{CRUDBooster::mainpath('edit-save/'.$jobOrder->id)}}'>
						...
						<div class='col-md-3'>
							<select class='form-control multi-select' name='company_id' id='company_id' onchange="listContacts(this.value)">
								@foreach($companies as $company)
									<option value='{{$company->id}}' {{($company->id == $jobOrder->company_id)?'selected':''}}>{{$company->name}}</option>
								@endforeach
							</select>
						</div>
						<div class='col-md-2'> 
						...
						<div class='col-md-3'>
							<select class='form-control' name='contact_id' id='contact_id'>
								@foreach($contacts as $contact)
									<option value='{{$contact->id}}' {{ ($contact->id == $jobOrder->contact_id) ? 'selected' : '' }}>{{$contact->first_name}}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class='col-md-3'>
							<input type='text' class='form-control' name='title' value='{{$jobOrder->title}}' id='title'>
						</div>
						<div class='col-md-2'> 
						...
						<div class='col-md-3'>
							<select class='form-control' name='industry' id='industry'>
								...
								@foreach($industries as $industry)
									<option value='{{$industry->id}}' {{($industry->id == $jobOrder->industry)?'selected':''}}> {{$industry->name}}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group">					
						...
						<div class='col-md-3'>
							...
							@foreach($functionalAreas as $functionalArea)
								<option value='{{$functionalArea->id}}'
									{{ (in_array($functionalArea->id, $jobOrder->functionalAreaIds) )? 'selected':''}}>
									{{$functionalArea->name}}
								</option>
							@endforeach
							</select>
						...
						<div class='col-md-3'>
							<select class='form-control multi-select' name='functional_area_skill[]' id='functional_area_skill' multiple="">
								@foreach($functionalAreaSkills as $functionalAreaSkill)
									<option value='{{$functionalAreaSkill->id}}'  {{ (in_array($functionalAreaSkill->id, $jobOrder->functionalAreaSkillIds) )? 'selected':''}} >
										{{$functionalAreaSkill->name}}
									</option>
								@endforeach
							</select>
						</div>
					...
					<div class="form-group">					
						...
						<div class='col-md-3'>
							<select class='form-control' name='functional_area_role' id='functional_area_role'>
								@foreach($functionalAreaRoles as $functionalAreaRole)
									<option value='{{$functionalAreaRole->id}}' {{ ($jobOrder->functional_area_role_id == $functionalAreaRole->id)?'selected':''}}>
										{{$functionalAreaRole->name}}
									</option>
								@endforeach
							</select>
						</div>
						...
						<div class='col-md-3'>
							<select class='form-control multi-select' name='general_skill[]' id='general_skill' multiple="">
								@foreach($generalSkills as $generalSkill)
									<option value='{{$generalSkill->id}}' {{ (in_array($generalSkill->id, $jobOrder->generalSkillIds) )? 'selected':''}} > {{$generalSkill->name}}
									</option>
								@endforeach
							</select>
						</div>
					...
					<div class="form-group">
						...
						<div class='col-md-3'>
							<input type='date' class='form-control' name='start_date' value='{{$jobOrder->start_date}}' id='start_date'>
						</div>
						...
						<div class='col-md-3'>
							<input type='date' class='form-control' name='expiry_date' value='{{$jobOrder->expiry_date}}' id='expiry_date'>
						</div>
					</div>
					...
					<!-- <div class="form-group">					
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='min_exp_years' value='{{$jobOrder->min_exp_years}}' id='min_exp_years'>
						</div>
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='min_exp_months' value='{{$jobOrder->min_exp_months}}' id='min_exp_months'>
						</div>
					</div>
					 -->
										<div class="form-group">
										...
										<input type="hidden" name='min_exp_years' id='min_exp_years' class="form-control" value="{{$jobOrder->min_exp_years}}">
										<input type="hidden" name='min_exp_months' id='min_exp_months' class="form-control" value="{{$jobOrder->min_exp_months}}">
										<input type="hidden"  id='min_exp_years1' class="form-control" value="{{$jobOrder->min_exp_years}}">
										<input type="hidden" id='min_exp_months1' class="form-control" value="{{$jobOrder->min_exp_months}}">
										<div class="col-md-3">
										                                       <div class="dropdown">
    <button class="btn btn-experience form-control" id="min_experience" onclick="getMinExpVal()" type="button" data-toggle="dropdown" value="{{$jobOrder->min_exp_years}} years {{$jobOrder->min_exp_months}} months">{{$jobOrder->min_exp_years}} years {{$jobOrder->min_exp_months}} months
...
@for($x = 0; $x <= 50; $x++)  
    <li>
<div class="radio">
  <label><input type="radio" value="{{$x}}" {{($jobOrder->min_exp_years==$x)?'checked="checked"':''}} name="min_year" onclick="getMinExpVal()">{{$x}}</label>
</div></li>
    @endfor
     </div>
      <div class="col-md-6 btn-scroll">
...
@for($x = 0; $x <= 11; $x++) 
      <li>
<div class="radio">
  <label><input type="radio" value="{{$x}}" {{($jobOrder->min_exp_months==$x)?'checked="checked"':''}} name="min_month" onclick="getMinExpVal()">{{$x}}</label>
</div></li>
   @endfor
</div>
  <p class="pull-right">
        <a class="btn btn-danger btn-sm" id="min_experience_remove">
          <span class="glyphicon glyphicon-remove"></span> Clear Selection
					...
					<!-- <div class="form-group">
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='max_exp_years' value='{{$jobOrder->max_exp_years}}' id='max_exp_years'>
						</div>
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='max_exp_months' value='{{$jobOrder->max_exp_months}}' id='max_exp_months'>
						</div>
					</div> -->
										...
										<input type="hidden" name='max_exp_years' id='max_exp_years' class="form-control" value="{{$jobOrder->max_exp_years}}">
										<input type="hidden" name='max_exp_months' id='max_exp_months' class="form-control" value="{{$jobOrder->max_exp_months}}">
										<input type="hidden"  id='max_exp_years1' class="form-control" value="{{$jobOrder->max_exp_years}}">
										<input type="hidden" id='max_exp_months1' class="form-control" value="{{$jobOrder->max_exp_months}}">
										<div class="col-md-3">
										                                       <div class="dropdown">
    <button class="btn btn-experience form-control" id="max_experience" onclick="getMaxExpVal()" type="button" data-toggle="dropdown" value="{{$jobOrder->max_exp_years}} years {{$jobOrder->max_exp_months}} months">{{$jobOrder->max_exp_years}} years {{$jobOrder->max_exp_months}} months
...
@for($x = 0; $x <= 50; $x++)  
    <li>
<div class="radio">
  <label><input type="radio" value="{{$x}}" {{($jobOrder->max_exp_years==$x)?'checked="checked"':''}} name="max_year" onclick="getMaxExpVal()">{{$x}}</label>
</div></li>
    @endfor
     </div>
      <div class="col-md-6 btn-scroll">
...
@for($x = 0; $x <= 11; $x++) 
      <li>
<div class="radio">
  <label><input type="radio" value="{{$x}}" {{($jobOrder->max_exp_months==$x)?'checked="checked"':''}} name="max_month" onclick="getMaxExpVal()">{{$x}}</label>
</div></li>
   @endfor
</div>
  <p class="pull-right">
        <a class="btn btn-danger btn-sm" id="max_experience_remove">
          <span class="glyphicon glyphicon-remove"></span> Clear Selection
					...
					<div class="form-group">					
						...
						<div class='col-md-3'>
							<input type='number' step="0.01" class='form-control' name='min_ctc' value='{{$jobOrder->min_ctc}}' id='min_ctc' placeholder="In Lakhs" min="0">
						</div>
						<div class='col-md-2'> 
							Max CTC <span class='text-danger' title='This field is required'>*</span>
						</div>
						<div class='col-md-3'>
							<input type='number' step="0.01" class='form-control' name='max_ctc' value='{{$jobOrder->max_ctc}}' id='max_ctc' placeholder="In Lakhs" min="0">
						</div>
					</div>
					...
					<!-- <div class="form-group">
						...
						<div class='col-md-3'>
							<select class='form-control' name='status' value='{{$jobOrder->status}}' id='status'>
								<option value=''>--</option>
 ...
 -->
						...
						<div class='col-md-3'>
							<select class='form-control' name='next_step' value='{{$jobOrder->next_step}}' id='nex_step'>
								<option value="">--</option>
							</select>
						</div> -->
					<!-- </div> -->
					...
					<!-- <div class="form-group">
						...
						<div class='col-md-3'>
							<input type='date' class='form-control' name='intro_call_date' value='{{$jobOrder->intro_call_date}}' id='intro_call_date'>
						</div>
						<div class='col-md-2'> 
							Submission Date<span class='text-danger' title='This field is required'>*</span>
						</div>
						<div class='col-md-3'>
							<input type='date' class='form-control' name='submission_date' value='{{$jobOrder->submission_date}}' id='submission_date'>
						</div>
					</div> -->
					...
					<div class="form-group">
						...
						<div class='col-md-3'>
							<input type='text' class='form-control' name='client_job_id' value='{{$jobOrder->client_job_id}}' id='client_job_id'>
						</div>
						<div class='col-md-2'> 
						...
						<div class='col-md-3'>
							<select class='form-control' name='type' id='type'>
								<option value='C'{{ ($jobOrder->type == 'C' )? 'selected' : '' }}> C </option>
								<option value='H' {{ ($jobOrder->type == 'H' )? 'selected' : '' }}> H </option>
							</select>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='duration' value='{{$jobOrder->duration}}' id='duration' placeholder="In Months" min="0">
						</div>
						<div class='col-md-2'> 
							Max Rate<!-- <span class='text-danger' title='This field is required'>*</span> -->
						</div>
						<div class='col-md-3'>
							<input type='text' class='form-control' name='rate_max' value='{{$jobOrder->rate_max}}' id='rate_max'>
						</div>
					</div>
					<div class="form-group">
						<div class='col-md-2'> 
						...
						<div class='col-md-3'>
							<input type='checkbox' name='is_hot'  id='is_hot' value='1' {{($jobOrder->is_hot == 1) ? 'checked':''}}>
						</div>
						<div class='col-md-2'> 
							Public
						</div>
						<div class='col-md-3'>
							<input type='checkbox' name='public' id='public' value='1' {{($jobOrder->public  == 1) ? 'checked':'' }}>
						</div>						
					</div>
					<div class="form-group">
						<div class='col-md-2'> 
						...
						<div class='col-md-3'>
							<select class='form-control multi-select' name='state' value='{{$jobOrder->state}}' id='state' onchange='listCities(this.value)'>
								<option value=''>--</option>
								@foreach($indianStates as $state)
									<option value='{{$state->id}}' {{ ($jobOrder->state == $state->id)? 'selected' : '' }}>
										{{$state->name}}
									</option>
								@endforeach
							</select>
						...
						<div class='col-md-3'>
							<select class='form-control multi-select' name='city' value='{{$jobOrder->city}}' id='city'>
								@foreach($cities as $city)
									<option value='{{$city->id}}' {{ ($jobOrder->city == $city->id)? 'selected' : '' }} >{{$city->name}}</option>
								@endforeach
							</select>
						</div>
					</div>
					<!-- <div class="form-group">
						...
						<div class='col-md-3'>
							<input type='text' class='form-control' name='company_department_id' value='{{$jobOrder->company_department_id}}' id='company_department_id'>
						</div>
						<div class='col-md-2'> 
							is_admin_hidden<span class='text-danger' title='This field is required'>*</span>
						</div>
						<div class='col-md-3'>
							<input type='text' class='form-control' name='is_admin_hidden' value='{{$jobOrder->is_admin_hidden}}' id='is_admin_hidden'>
						</div>
					</div> -->
					...
					<div class="form-group">
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='num_vacancies' value='{{$jobOrder->num_vacancies}}' id='num_vacancies' min = '1'>
						</div>
						...
						<div class='col-md-3'>
							<input type='number' onkeypress="return checkIsNumber(event)" class='form-control' name='openings_available' value='{{$jobOrder->openings_available}}' id='openings_available'>
						</div> -->
					</div>
					...
					<div class="form-group">
						...
						<div class='col-md-8'>
							<textarea class='form-control' name='notes' id='notes' rows="6">{{$jobOrder->notes}}</textarea>
						</div>
					</div>
					<div class="form-group">
						<div class='col-md-2'> 
						...
						<div class='col-md-8'>
							<textarea class='form-control jobOrderDescription' name='description' id='description' rows="6">
					...
					<div class="form-group">					
						...
						<div class='col-md-3'>
							...
							<option value='{{$user->id}}' {{ $user->id == $jobOrder->owner ? 'selected':''}} > {{$user->name}}
							</option>
								@endforeach
						...
						<div class='col-md-3'>
							<select class='form-control' name='recruiter' id='recruiter'>
								<option value=''>--</option>
								@foreach($users as $user)
							<option value='{{$user->id}}' {{ $user->id == $jobOrder->recruiter ? 'selected':''}} > {{$user->name}}
							</option>
								@endforeach
							</select>
						</div>
...
<script src="http://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.11/summernote.js"></script>
	<script type="text/javascript">
		...
		$( "#min_experience_remove" ).click(function() {
				var ayears= $('#min_exp_years1').val();
	    		var amonth= $('#min_exp_months1').val();
	    		var buttonValue=ayears+' years'+' '+ amonth+' months';
	   		$('#min_experience').html(buttonValue);
	   		$('#min_experience').val(buttonValue);
	   		$('#min_exp_years').val("");
	   		$('#min_exp_months').val("");
	   		$('#min_exp_years').val(ayears);
	   		$('#min_exp_months').val(amonth);
	        $("input[name=min_year][value=" + ayears + "]").prop('checked', true);
	        $("input[name=min_month][value=" + amonth + "]").prop('checked', true);    
		});
		$('#description').summernote({
			...
			var ayears= $('#max_exp_years1').val();
    		var amonth= $('#max_exp_months1').val();
    		var buttonValue=ayears+' years'+' '+ amonth+' months';
   		$('#max_experience').html(buttonValue);
   		$('#max_experience').val(buttonValue);
   		$('#max_exp_years').val("");
   		$('#max_exp_months').val("");
   		$('#max_exp_years').val(ayears);
   		$('#max_exp_months').val(amonth);
        $("input[name=max_year][value=" + ayears + "]").prop('checked', true);
        $("input[name=max_month][value=" + amonth + "]").prop('checked', true);
      ...
      $('#max_experience').val(buttonValue);
		...
		function listContacts(_companyId){
			$.get('{{CRUDBooster::mainpath()}}/add', {
                ...
                select.append('<option value=' + value.id + '>' + value.first_name +' '+ value.last_name +
	                	'</option>');
	            });
			});
		}
